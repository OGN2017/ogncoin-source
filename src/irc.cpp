...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #OGNCOINTEST2\r");
            Send(hSocket, "WHO #OGNCOINTEST2\r");
        } else {
            // randomly join #OGNCOIN00-#OGNCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #OGNCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #OGNCOIN%02d\r", channel_number).c_str());
        }

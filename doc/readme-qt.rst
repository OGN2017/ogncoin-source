OGNCOIN-qt: Qt4 GUI for OGNCOIN
===============================
...
Alternatively, install Qt Creator and open the `OGNCOIN-qt.pro` file.

An executable named `OGNCOIN-qt` will be built.
...
A warning for people using the *static binary* version of OGNCOIN on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of OGNCOIN is linked against libdb5.3.
...
installed causes OGNCOIN-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
